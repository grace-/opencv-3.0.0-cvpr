...
int main(int argc, char *argv[]) {
  ...
  square = atof(argv[1]);
  {
    ...
    num_arucos = aruco_board_config.size();
    aruco_board_3f.reserve(num_arucos);
    std::vector<cv::Point3f> aruco_marker_3f(4);

    for (int i = 0; i < num_arucos; ++i) {
  ...
  while(video_stream[0].read(frame[0])) {
    ...
    if (keypress == 32) { 
      if (use_fisheye_model) {
        if (calibrated &&
            ((single_rig_detections_id[0].size() == num_arucos) &&
             (single_rig_detections_id[cam_calib_idx].size() == num_arucos))) {
          ...
          num_calib++;              
        } else if (single_rig_detections_id[cam_calib_idx].size() == num_arucos) {
          all_rig_detections_2f[cam_calib_idx]
              .push_back(single_rig_detections_2f[cam_calib_idx]);
          all_rig_detections_3f[cam_calib_idx]
              .push_back(single_rig_detections_3f[cam_calib_idx]);
    ...
    if (keypress == 10) {
      std::cout << "--------------------------------------------------------\n";
      if ((use_fisheye_model && (all_rig_detections_2f[cam_calib_idx].size() > 0)) ||
        ...
        if (!calibrated) {
          if (use_fisheye_model) {
            ...
            std::cout << "fisheye camera " << cam_calib_idx << ": " << std::endl
        ...
        if (calibrated && (num_cameras > 1)) { 
          if (use_fisheye_model) {
            ...
            cv::Vec3d t;
            cv::Vec4d D1, D2;
            rms = cv::fisheye::stereoCalibrate(all_rig_detections_3f[0],
                                               ...
                                               R, t, flag);
            cv::Rodrigues(R, om);
            std::cout << "Fisheye camera stereo pair (0, " << cam_calib_idx++ << "):" << std::endl
                      << "  R = " << R << std::endl
                      << " om = " << om.t() << std::endl
                      << "  t = " << t.t() << std::endl
                      << "  rms = " << rms << std::endl;
            ...
            // For each stereo pair, find indices of co-detected aruco markers for each capture
            for (int i = 1; i < num_cameras; ++i) {
